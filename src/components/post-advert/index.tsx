...
import { formatTimestamp, isValidDateRange, isUnderGivenDuration, isStartAvailable } from "../../utils/utils";
...
const PostAdvert: React.FC<PostAdvertProps> = () => {
  ...
  const handleSubmit = () => {
    ...
    if (availDate.length > 0 && !isStartAvailable(startDate, availDate))
      return toast.error(`Another user advertisement scheduled till ${availDate}`);
